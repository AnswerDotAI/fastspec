...
# %% auto #0
__all__ = ['sanitized_params', 'mk_sig', 'mk_doc', 'OpFunc', 'SyncOpFunc', 'OpGroup', 'OpenAPIClient']
...
def mk_sig(op, sparams, defaults=None):
    "Create a compact operation signature from an `OpSpec`; `defaults` values make their params optional."
    defaults = defaults or {}
    params = []
    for pname, sname in sparams.items():
        params.append(_mk_param(sname, pname in op.required_params, op.param_types.get(pname), defaults.get(pname, op.param_defaults.get(pname))))

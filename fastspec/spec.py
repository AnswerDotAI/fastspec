...
Docs: https://AnswerDotAI.github.io/fastspec/spec.html.md"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/03_spec.ipynb.

# %% auto #0
__all__ = ['ctypes', 'OpSpec', 'snake', 'sanitize_param_name', 'openapi_to_ops', 'discovery_to_ops', 'SpecParser']
...
# %% ../nbs/03_spec.ipynb #653fb26f
def sanitize_param_name(p): return snake(re.sub(r'\W', '_', p).strip('_'))

# %% ../nbs/03_spec.ipynb #cb91b9f0
def _group_name(op_id: str, path: str, verb:str='', path_tags:list=None, op_tags:list=None):
...
def _op_docs_url(op):
    "Docs URL for an op: prefer explicit `externalDocs`, else first URL in the description."
    if (ed := op.get("externalDocs")) and ed.get("url"): return str(ed["url"])
...
def _plain(v):
    ...
    if isinstance(v, (list,tuple,L)): return [_plain(o) for o in v]
    if isinstance(v, dict): return {k:_plain(o) for k,o in v.items()}
    return v

# %% ../nbs/03_spec.ipynb #112ac323
def _dedupe_params(pdict, bpdict):
    'If query/body params collide, prefer body params for the Python signature.'
    route_s = {sanitize_param_name(p) for p in pdict.route_params}
    bpdict.body_params = [p for p in bpdict.body_params if sanitize_param_name(p) not in route_s]
    body_s = {sanitize_param_name(p) for p in bpdict.body_params}
    pdict.query_params = [p for p in pdict.query_params if sanitize_param_name(p) not in route_s|body_s]
    return pdict, bpdict
...
class SpecParser:
    ...
    def to_dict(self):
        "Compact plain-data form (empty fields dropped, types as names); see `from_dict` and `save`."
        def _op(o):
            d = {k:v for k,v in asdict(o).items() if v}
            if 'param_types' in d: d['param_types'] = {k:t.__name__ for k,t in d['param_types'].items()}
            return d
    ...
    def save(self, path):
        "Write `to_dict` form as a Python module defining `spec`: compact, git-diffable, shippable in client packages."
